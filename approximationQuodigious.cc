...
template<u64 length>
inline void innerBody(std::ostream& stream, u64 product, u64 index, u64 depth) noexcept;
template<u64 length>
inline void body(std::ostream& stream, u64 product = 1, u64 index = 0, u64 depth = 0) noexcept {
	...
	if (length >= 11) {
		...
		auto lowerHalf = std::async(std::launch::async, [baseProduct](auto product, auto index, auto depth) noexcept {
				std::ostringstream stream;
				product += baseProduct;
				index += next;
				innerBody<inner>(stream, product, index, depth); // 3
				product += baseProduct;
				index += next;
				innerBody<inner>(stream, product, index, depth); // 4
                product += (baseProduct << 1);
                index += (next << 1);
                innerBody<inner>(stream, product, index, depth); // 6
				return stream.str();
				}, product, index, depth);
		auto upperHalf = std::async(std::launch::async, [baseProduct](auto product, auto index, auto depth) noexcept {
					std::ostringstream stream;
					product += ((baseProduct << 2) + baseProduct);
					index += ((next << 2) + next);
					innerBody<inner>(stream, product, index, depth); // 7
					product += baseProduct;
					index += next;
					innerBody<inner>(stream, product, index, depth); // 8
					product += baseProduct;
					index += next;
					innerBody<inner>(stream, product, index, depth); // 9
					return stream.str();
				}, product, index, depth);
        ...
        innerBody<inner>(stream, product, index, depth); // 2
...
inline void innerBody(std::ostream& stream, u64 product, u64 index, u64 depth) noexcept {
    ...
    body<length>(stream, product, index, depth + 1);
}
template<>
inline void innerBody<0>(std::ostream& stream, u64 product, u64 index, u64 depth) noexcept {
    if (index % product == 0) {
